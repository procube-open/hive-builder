...
- hive-certificate
サービスを構築する上で、複数枚の証明書が必要な場合があります。hive_builderでは変数certificatesを指定することで任意のドメイン名、サフィックス、有効期限で証明書の生成が可能です。
詳細は :ref:`こちら<cashare-create>` を確認ください。

- hive-trust-ca
CA局証明書をコンテナのトラストストアにコピーします。
また、上記ビルトインロールhive-certificateで生成したクライアント(サーバー)の証明書と秘密鍵についても、コンテナのトラストストアへコピーを行います。
