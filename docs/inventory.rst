...
共有リポジトリサーバ
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
複数のhive群を監視可能な共有レポジトリサーバを構築することができます。
共有レポジトリサーバを構築する場合は、shared_repository プロパティに辞書型で以下の属性を指定してください。
...
..  list-table::
    ...
    * - context_dir
      - /var/lib/hive
      - 共有レポジトリサーバのコンテキストディレクトリ。設定は必須です。
    * - ssh_port
      - 8022
      - 共有レポジトリサーバのSSHポート。デフォルトは2222です。
    * - dns_address
      - 8.8.8.8
      - 共有レポジトリサーバが使用するDNSサーバのIPアドレス。デフォルトは/etc/resolv.confに記載されたアドレスを取得します。

shared_repository が設定されている場合、レポジトリサーバはマザー上にdockerコンテナとして構築されます。
...
共有レポジトリサーバは内部でdnsdistを起動しており、これを用いてhiveサーバへの名前解決を行います。
/etc/extra-hostsには、共有レポジトリサーバが管理する全てのhiveサーバの名前とIPアドレスが登録されており、これ以外の名前解決にはマザー上の/etc/resolv.confに記載されたDNSサーバを使用します。

**utility command**

通常、dsh,dexec などのコマンドは、最後にsetup-hostsが実行されたhive群のサービスに対して接続されます。
...
外部ネットワークを経由してsyslogを受信する場合は、通常のudpではなく、TCP+TLSを使用することを推奨します。
そのために、以下のコマンドを実行してTLSを有効化して下さい。

::

  hive set tls_logging true

**その他注意事項**

- hive群を跨いでサービス名やバックアップ名に被りがある場合、正しく動作しない場合があります。
- build-infra -H などでサーバをダウンさせた場合、共有レポジトリサーバもダウンします。他のhive群の監視を続けたい場合はコンテナ収容サーバのみをダウンさせてください。
- prepared で共有レポジトリサーバを構築する場合はマザーサーバのIPアドレスも含めてip_address_listに指定してください。
...
..  list-table::
    ...
    * - cpu_limit
      - 1.5
      - 省略
      - サービスに割り当てる CPU の上限値（docker service create の --limit-cpu オプションと等価）
    * - mem_limit
      - 512M
      - 省略
      - サービスに割り当てるメモリの上限値（docker service create の --limit-memory オプションと等価）
