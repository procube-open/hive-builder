...
hive-builder リポジトリでは github actions により PyPI, test PyPI への自動デプロイが入ります。

## 10-1. PyPI へのデプロイ仕様

PyPI へのデプロイ仕様は以下の通りです。

- main ブランチへのプルリクエストのマージ発生時に実行されます
- 以下の仕様に従って、適切なバージョン番号が導出されます
  - コミットログ内に `[[MAJOR]]` が含まれる場合、メジャーバージョンのアップデートを行います
  - コミットログ内に `[[MINOR]]` が含まれる場合、マイナーバージョンのアップデートを行います
  - 上記以外の場合は、パッチバージョンのアップデートを行います
- 上記で導出したバージョン番号に従って、`pyproject.toml` の更新、git のタグ付けを行い、コミットします
- その後、ビルドと PyPI へのデプロイを実施します

## 10-2. Test PyPI へのデプロイ仕様（RC版）

Test PyPI へのデプロイ仕様は以下の通りです。

- main ブランチ以外のブランチへのプッシュについて、コミットログ内に `[[PRERELEASE]]` が含まれる場合に実行されます
- そのブランチ内の初回の実行の場合、patch レベルを1つ上げ、 rc{番号} が後ろにつく形でバージョンアップを行います
- 同じブランチに複数回 `[[PRERELEASE]]` を含むコミットをした場合、バージョン番号は変わらず、回数分 rc 後の番号が上がります
- 上記で導出したバージョン番号に従って、`pyproject.toml` の更新、git のタグ付けを行い、コミットします
- その後、ビルドと Test PyPI へのデプロイを実施します

### 注意事項

複数のブランチでプレリリースを実行するとバージョン番号が被ってしまい、エラーが発生してしまいます。

複数のブランチでプレリリースを実施する場合は、`pyproject.toml` の `version` 変数を編集し、バージョン番号が被らないようにすることで回避できます。
